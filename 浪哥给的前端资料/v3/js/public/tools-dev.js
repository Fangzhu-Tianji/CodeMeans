...
define(function (require, exports, modules) {
    ...
    T.ajax=function (url,option){
        ...
        setting=$.extend(true,{url:options.apiUrl+url,type:options.type},options);
        setting.success=function(data){       
            if(data && data.errno!=0){
              switch(data.errno){
                case 30 :
                    (T.login.isJump() || options.loginJump) ? (location.href = T.site.passport + "login.html?jump=" + window.location.href) : T.login.openLogin();
                  break;
                case 100100:
				default:
              ...
              options.error && jQuery.isFunction(options.error) && options.error.call(null,{errno:data.errno,errmsg:data.errmsg}); 			  
              return ;
            }
            options.success && jQuery.isFunction(options.success) && options.success.call(null,data.data);
        };
        var iframeAjax = function(setting) {
            var queueName = options.apiUrl.replace("http://","").replace("/api/","").replace(/\./g, '_');
            ...
            if (!(ifrm.length)) {
                ...
                $('body').append(ifrm);
                ifrm[0].src = options.apiUrl.replace("/api/","") +'/crossdomain.html';
            }
        };
		if ($.support.cors) {
			$.ajax(setting)
   ...
   /* 弹窗登录  */	
	T.login.openLogin=function(){
		layer.open({
			type: 2,
			title: '您尚未登录',
			area: ['400px', '410px'],
			shade: [0.8, '#393D49'],
			content: T.site.passport+'iframeLogin.html'
		}); 
	};
   /* 退出登录 */
