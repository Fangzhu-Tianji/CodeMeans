...
define(function (require, exports, modules) {
    ...
    // 当前域名，得到如：ibo.admin.ccdfs.cc
    var _HOSTNAME = location.hostname,
        // 最后一个点号的位置 15 - 5 = 10 ，截取从第10个字符到最后字符 得到当前域：ccdfs.cc 
    ...
    T.cookie = function (key, value, day) {
        ...
        if (typeof (key) != 'undefined' && typeof (value) != 'undefined') {
            if (day) {
                ...
                $.cookie(key, value, { path: '/', domain: T.DOMAIN.DOMAIN });
            }
            ...
            if (options.data.errno == 0 && T.Typeof(options.data.data, /Object/)) {
                if (options.data.data.errno == 0) {
                    ...
                    if (options.failure) {
						...
						window.layer && layer.alert(options.data.data.errmsg,{icon:2});
					}
                } else {
                    if (options.error) {
        ...
        var ajaxOptions = {
            ...
            success: function (data) {
                ...
                if (configs.initData.ResultList && configs.initData.ResultList.length > 0) {
                    ...
                    // 没有数据！
                    $(configs.target).html("");
                    $(configs.pageBar).html('<p class="noDataBack">没有找到你要的数据！</p>');
                }
                // 显示分页条
    ...
    T.getCustomString = function (str, len, preStr) {
        var newLength = 0;
        var newStr = "";
     ...
     * @param len: 需要截取的长度 
     */
    T.cutCustomString = function (str, len) {
        var str_length = 0;
        var str_len = 0;
        str_cut = new String();
        str_len = str.length;
    ...
    */
    T.pageBar = function (_total, _per, _pn) {
     ...
     * 
     */
    T.getImgSize = function (sUrl, callback) {
        var img = new Image();
    ...
    // 金钱转成字符串显示
    // 如1000000，转成  1,000,000
    T.fmtMoney = function (money) {
        var intpart = parseInt(money);
        var decpart = (money - intpart).toFixed(2);
        var str = intpart.toString();
        var arrs = [];
        if (str.length >= 3) {
            for (var i = 1; i <= str.length / 3; i++) {
                arrs.push(str.substr(-3 * i, 3));
            }
            if (str.length % 3 != 0) {
                arrs.push(str.substr(0, str.length % 3));
            }
        } else {
            arrs.push(str.substr(0, str.length % 3));
        }

        return arrs.reverse().join(",") + decpart.toString().substring(1);
    }
    ...
    var maction = {
        // 注册关闭事件
        addEvent: function () {
            $('body').on('click', '.maction-close', function () {
                var pop = $(this).closest('.maction').remove();
            });
        },
        success: function (msg, fn) {
            maction.addEvent();
            var html = "<div class='maction success-maction'><div class='olay'></div><div class='cnt'><span>" + msg + "</span></div></div>";
            $(html).appendTo($('body'));
            setTimeout(function () {
                if (fn) {
                    fn();
                }
                $('.success-maction').fadeOut(500, function () {
                    $(this).remove();
                });
            }, 2000);
        },
        fail: function (msg, fn) {
            maction.addEvent();
            var html = "<div class='maction fail-maction'><div class='alllay'></div><div class='olay'></div><div class='cnt'><div class='header'>温馨提示<a href='javascript:;' class='maction-close'></a></div><span>" + msg + "</span></div></div>";
            $(html).appendTo($('body'));
            setTimeout(function () {
                if (fn) {
                    fn();
