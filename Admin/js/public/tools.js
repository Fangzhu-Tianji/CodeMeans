...
; (function (window, document, undefined) {
    ...
    // 清除 cookie 设置过期时间
    T.uncookie = function () {
        // $.cookie("__sid", "", { expires: -1, path: '/', domain: T.DOMAIN.DOMAIN });
        $.cookie("__sid", "", { expires: -1, path: '/' });
    ...
    T.GET = function (options, _failure, _error) {
        ...
        if (!/^http/.test(options.action)) {
            ...
            options.action = T.DOMAIN.WWW + options.action;
        }
    ...
    // 后加的分页表格
    T.getRequestAfter = function (options) {
        ...
        if (window.T.GET && typeof (window.T.GET) == 'function') {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // 显示列表
                    // var targetTag = configs.target[0].tagName.toLowerCase();  == tbody
                    $(configs.target).html($(configs.tmpl).tmpl(data));
                    $(configs.target).children(":odd").addClass("odd");
                    $(configs.target).children().hover(function () {
                        $(this).addClass("over");
                    }, function () {
                    ...
                    // 显示分页条
                    // var pageBar = createPageBar(configs.page.TotalRecord, configs.page.PageSize, configs.page.PageIndex)
                    // $(configs.pageBar).html(pageBar);
                    // // 绑定事件
                    // $(configs.pageBar).find("a").off("click").on("click", function () {
                    //     var params = options;
                    //     params.page.PageIndex = $(this).attr("data-pn");
                    //     T.getRequestAfter(params);
                    // });
                },
    ...
    T.setPage = function (options) {
        ...
        var configs = {
            ...
            initData: {
                ResultList: [
                    {"userid": 10602,"nick":"张三"},
                    {"userid": 10602,"nick":"五武"}
                ],
                "AllowPaging": true,
                "PageIndex": 1,
                "PageSize": 15,
        ...
        var pageBar = createPageBar(configs.initData.TotalRecord, configs.initData.PageSize, configs.initData.PageIndex)
        $("#tfpagebar").html(pageBar);
        // 绑定事件
        $("#tfpagebar").find("a").off("click").on("click", function () {
            var params = options;
            params.page.PageIndex = $(this).attr("data-pn");
            T.setPage(params);
        });
    ...
    T.popup = function (options) {
        ...
        var defaults = {                        // 创建蒙板可更改参数
            ...
            baseCssLink: T.DOMAIN.WWW + '/admin_cst_4.0/css/public/m-popup.css',
